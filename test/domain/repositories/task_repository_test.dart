...
void main() {
  ...
  group('TaskRepository', () {
    ...
    test('should get task by id', () async {
      when(mockTaskRepository.getTaskById(1)).thenAnswer((_) async => testTask);
      final result = await mockTaskRepository.getTaskById(1);
      expect(result, testTask);
    });

    test('should create task', () async {
      when(mockTaskRepository.createTask(testTask)).thenAnswer((_) async => {});
    ...
    test('should soft delete task', () async {
      when(mockTaskRepository.softDeleteTask(1)).thenAnswer((_) async => {});
    ...
    test('should get unsynced tasks', () async {
      when(mockTaskRepository.getUnsyncedTasks()).thenAnswer((_) async => [testTask]);
      final result = await mockTaskRepository.getUnsyncedTasks();
      expect(result, [testTask]);
    });

    test('should mark task as synced', () async {
      when(mockTaskRepository.markTaskAsSynced(1)).thenAnswer((_) async => {});
